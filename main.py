...
# ─────────────────────────────────────────
TOKEN         = "8730226032:AAG9pWap8BG8nTWTnJErH3k-UVmdU6wMPeM"       # Paste your bot token
ADMIN_ID_RAW  = 8379062893                  # Your Telegram user ID
ADMIN_CONTACT = "https://t.me/EXUCODR"     # Your contact link
# ─────────────────────────────────────────

if not TOKEN or TOKEN == "YOUR_BOT_TOKEN_HERE":
    print("FATAL: Please set your TOKEN in the script configuration block.")
    exit(1)
...
ADMIN_CONTACT_LINK = ADMIN_CONTACT
...
SCHEDULED_FILES_CONFIG = os.path.join(DATA_DIR, 'scheduled_files.json') # New config file
...
def generate_jwt(uid, pas):
    ...
    try:
        g_dat = {"uid": uid, "password": pas, "response_type": "token", "client_id": CLIENT_ID, "client_secret": CLIENT_SECRET, "client_type": "2"}
        r_grant = requests.post(OAUTH_URL, data=g_dat, timeout=30)
        if r_grant.status_code != 200:
            return None, None, f"OAuth Failed: HTTP {r_grant.status_code}"
        grant_json = r_grant.json()
        acc, oid = grant_json.get("access_token"), grant_json.get("open_id")
        plat, region = grant_json.get("platform", 4), grant_json.get("region")
        if not acc or not oid:
            return None, None, "OAuth Failed: Missing token"
        raw_pb = build_major_pro(oid, acc, uid, plat)
        r_login = requests.post(MAJOR_URL, data=aes_enc(raw_pb), headers=headers, timeout=45)
        if r_login.status_code != 200:
            return None, None, f"Major Login Failed: HTTP {r_login.status_code}"
        dec = aes_dec(r_login.content)
        pb_data, _ = blackboxprotobuf.decode_message(dec)
        jwt_token = pb_data.get('8')
        if isinstance(jwt_token, bytes):
            jwt_token = jwt_token.decode(errors='ignore')
        if not jwt_token:
            return None, None, "No JWT token"
        return jwt_token, region, None
    except Exception as e:
...
def is_user_vip(user_id: int) -> bool:
    """Checks if a user is currently a VIP. Admin is always VIP."""
    if user_id == ADMIN_ID:
        return True
...
COMMAND_BUTTONS_LAYOUT = [
    ["📤 ᴘʀᴏᴄᴇss ꜰɪʟᴇ", "📇 ᴠɪᴘ sᴛᴀᴛᴜs"],
    ["🛒 ᴠɪᴘ sʜᴏᴘ", "📊 ɢɪᴛʜᴜʙ sᴛᴀᴛᴜs"],
    ["⚙️ sᴄʜᴇᴅᴜʟᴇᴅ ꜰɪʟᴇs", "🆘 ʜᴇʟᴘ"],
    ["❌ ᴄᴀɴᴄᴇʟ"]
]

# Admin-only buttons (extra controls)
ADMIN_BUTTONS_LAYOUT = [
    ["📤 ᴘʀᴏᴄᴇss ꜰɪʟᴇ", "📇 ᴠɪᴘ sᴛᴀᴛᴜs"],
    ["🛒 ᴠɪᴘ sʜᴏᴘ", "📊 ɢɪᴛʜᴜʙ sᴛᴀᴛᴜs"],
    ["⚙️ sᴄʜᴇᴅᴜʟᴇᴅ ꜰɪʟᴇs", "🆘 ʜᴇʟᴘ"],
    ["👑 ᴠɪᴘ ᴍᴀɴᴀɢᴇ", "📢 ʙʀᴏᴀᴅᴄᴀsᴛ"],
    ["❌ ᴄᴀɴᴄᴇʟ"]
]

def get_markup(user_id: int) -> ReplyKeyboardMarkup:
    """Return admin or user keyboard based on user_id."""
    layout = ADMIN_BUTTONS_LAYOUT if user_id == ADMIN_ID else COMMAND_BUTTONS_LAYOUT
...
async def start(update: Update, context: CallbackContext) -> None:
    """Send welcome message with profile photo - exact format only."""
    ...
    tg_username = f"@{user.username}" if user.username else "ɴ/ᴀ"
    now_bst = datetime.now(timezone.utc) + timedelta(hours=6)
    time_str = now_bst.strftime("%I:%M:%S %p") + " ʙsᴛ"
    markup = get_markup(user_id)

    welcome_text = (
        f"✅ *{fancy('Welcome Successfully')}!*\n\n"
        f"👤 *ᴘʟᴀʏᴇʀ:* {fancy(username)}\n"
        f"🎮 *ɪᴅ:* {user_id}\n"
        f"👤 *ᴜsᴇʀ:* {tg_username}\n\n"
        f"⏰ *ᴛɪᴍᴇ:* {time_str}"
    )
...
async def help_command(update: Update, context: CallbackContext) -> None:
    ...
    help_text = (
        f"🆘 *{fancy('Help Center')}*\n\n"
        f"━━━━ 📌 *{fancy('General Commands')}* ━━━━\n"
        f"  `/start` — ᴍᴀɪɴ ᴡᴇʟᴄᴏᴍᴇ ᴍᴇssᴀɢᴇ\n"
        f"  `/help` — sʜᴏᴡ ᴛʜɪs ʜᴇʟᴘ ɢᴜɪᴅᴇ\n"
        f"  `/vipstatus` — ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴠɪᴘ sᴛᴀᴛᴜs\n"
        f"  `/vipshop` — ᴠɪᴇᴡ ᴠɪᴘ ᴘʟᴀɴs & ᴘʀɪᴄɪɴɢ\n"
        f"  `/cancel` — ᴄᴀɴᴄᴇʟ ᴀɴʏ ᴀᴄᴛɪᴠᴇ ᴏᴘᴇʀᴀᴛɪᴏɴ\n\n"
        f"━━━━ 🔧 *{fancy('VIP Commands')}* ━━━━\n"
        f"  `/setgithub <TOKEN> <owner/repo> <branch> <file.json>`\n"
        f"  `/mygithub` — ᴠɪᴇᴡ ʏᴏᴜʀ ɢɪᴛʜᴜʙ ᴄᴏɴꜰɪɢ\n"
        f"  `/setfile <Interval> <Name.json>` — sᴄʜᴇᴅᴜʟᴇ ᴀᴜᴛᴏ-ᴘʀᴏᴄᴇssɪɴɢ\n"
        f"     📏 *ꜰᴏʀᴍᴀᴛ:* `Xm` ᴍɪɴs · `Xh` ʜᴏᴜʀs · `Xd` ᴅᴀʏs\n"
        f"  `/removefile <Name.json>` — ʀᴇᴍᴏᴠᴇ ᴀ sᴄʜᴇᴅᴜʟᴇ\n"
        f"  `/scheduledfiles` — ʟɪsᴛ ᴀʟʟ sᴄʜᴇᴅᴜʟᴇs\n\n"
        f"━━━━ 👑 *{fancy('Owner / Admin Commands')}* ━━━━\n"
        f"  `/vip add <user_id> <days>` — ɢʀᴀɴᴛ ᴠɪᴘ\n"
        f"  `/vip remove <user_id>` — ʀᴇᴠᴏᴋᴇ ᴠɪᴘ\n"
        f"  `/vip list` — ᴠɪᴇᴡ ᴀʟʟ ᴀᴄᴛɪᴠᴇ ᴠɪᴘs\n"
        f"  `/broadcast <msg>` — ᴍᴇssᴀɢᴇ ᴀʟʟ ᴜsᴇʀs\n\n"
        f"━━━━ 📤 *{fancy('Manual Processing')}* ━━━━\n"
        f"  1️⃣ sᴇɴᴅ ᴀ JSON ꜰɪʟᴇ ᴡɪᴛʜ ᴜɪᴅ + ᴘᴀssᴡᴏʀᴅ ᴘᴀɪʀs\n"
        f"  2️⃣ ʙᴏᴛ ᴘʀᴏᴄᴇssᴇs & ʀᴇᴛᴜʀɴs ʀᴇsᴜʟᴛ ꜰɪʟᴇs\n"
        f"  3️⃣ ᴠɪᴘ ᴜsᴇʀs ɢᴇᴛ `jwt_token.json` ᴀᴜᴛᴏ-ᴜᴘʟᴏᴀᴅᴇᴅ\n\n"
        f"━━━━ ⚙️ *{fancy('Auto Processing (VIP)')}* ━━━━\n"
        f"  1️⃣ ᴜsᴇ `/setfile` ᴛᴏ ᴅᴇꜰɪɴᴇ sᴄʜᴇᴅᴜʟᴇ & ɴᴀᴍᴇ\n"
        f"  2️⃣ sᴇɴᴅ ᴛʜᴇ JSON ꜰɪʟᴇ ᴡʜᴇɴ ᴘʀᴏᴍᴘᴛᴇᴅ\n"
        f"  3️⃣ ʙᴏᴛ ʀᴜɴs ɪᴛ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴀᴛ sᴇᴛ ɪɴᴛᴇʀᴠᴀʟs\n"
    )
    await update.message.reply_text(
        help_text,
        parse_mode=ParseMode.MARKDOWN,
...
async def vip_shop_command(update: Update, context: CallbackContext) -> None:
    ...
    vip_shop_text = (
        f"🛒 *{fancy('VIP Shop')}*\n\n"
        f"✨ *{fancy('Unlock Premium Features')}:*\n"
        f"  ⚡ ᴀᴜᴛᴏ ɢɪᴛʜᴜʙ ᴛᴏᴋᴇɴ ᴜᴘʟᴏᴀᴅ\n"
        f"  🔁 sᴄʜᴇᴅᴜʟᴇᴅ ꜰɪʟᴇ ᴘʀᴏᴄᴇssɪɴɢ\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💼 *{fancy('Plans and Pricing')}:*\n\n"
        f"  🗓️  `  7 {fancy('Days')}  ` ━━━━ `₹ 10`\n"
        f"  🗓️  ` 15 {fancy('Days')}  ` ━━━━ `₹ 49`\n"
        f"  📅  `  1 {fancy('Month')} ` ━━━━ `₹ 69`\n"
        f"  📅  `  2 {fancy('Months')}` ━━━━ `₹ 89`\n"
        f"  📅  `  3 {fancy('Months')}` ━━━━ `₹ 99`\n"
        f"  🎯  `  1 {fancy('Year')}  ` ━━━━ `₹ 159`\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📩 *{fancy('To Purchase')}:*\n"
        f"👉 ᴄᴏɴᴛᴀᴄᴛ [Admin Here]({ADMIN_CONTACT_LINK})"
    )
...
async def vip_status_command(update: Update, context: CallbackContext) -> None:
    ...
    # Admin always shows as Lifetime VIP Owner
    if user_id == ADMIN_ID:
        now_bst = datetime.now(timezone.utc) + timedelta(hours=6)
        time_str = now_bst.strftime("%I:%M:%S %p") + " ʙsᴛ"
        tg_username = f"@{user.username}" if user.username else "ɴ/ᴀ"
        status_msg = (
            f"✅ *{fancy('Welcome Successfully')}!*\n\n"
            f"👤 *ɴᴀᴍᴇ:* {fancy(user.first_name or 'Admin')}\n"
            f"🎮 *ɪᴅ:* `{user_id}`\n"
            f"👤 *ᴜsᴇʀ:* {tg_username}\n"
            f"🏅 *sᴛᴀᴛᴜs:* 👑 ᴏᴡɴᴇʀ / ᴀᴅᴍɪɴ\n"
            f"♾️ *ᴇxᴘɪʀᴇs:* ʟɪꜰᴇᴛɪᴍᴇ\n"
            f"⏰ *ᴛɪᴍᴇ:* {time_str}\n\n"
            f"✅ ꜰᴜʟʟ ᴀᴄᴄᴇss ᴛᴏ ᴀʟʟ ꜰᴇᴀᴛᴜʀᴇs!"
        )
        try:
            photos = await context.bot.get_user_profile_photos(user_id, limit=1)
            if photos.total_count > 0:
                file_id = photos.photos[0][0].file_id
                await update.message.reply_photo(photo=file_id, caption=status_msg, parse_mode=ParseMode.MARKDOWN, reply_markup=main_reply_markup)
                return
        except Exception:
            pass
        await update.message.reply_text(status_msg, parse_mode=ParseMode.MARKDOWN, reply_markup=main_reply_markup)
        return
    ...
    if expiry_date_str and "Invalid" not in expiry_date_str:
        status_msg = (
            f"✅ *{fancy('Welcome Successfully')}!*\n\n"
            f"👤 *ɴᴀᴍᴇ:* {fancy(user.first_name or 'User')}\n"
            f"🎮 *ɪᴅ:* `{user_id}`\n"
            f"👤 *ᴜsᴇʀ:* {tg_username}\n"
            f"🏅 *sᴛᴀᴛᴜs:* 👑 ᴀᴄᴛɪᴠᴇ ᴠɪᴘ\n"
            f"📅 *ᴇxᴘɪʀᴇs:* `{expiry_date_str}`\n"
            f"⏰ *ᴛɪᴍᴇ:* {time_str}\n\n"
            f"✅ ʏᴏᴜ ʜᴀᴠᴇ ᴀᴄᴄᴇss ᴛᴏ ᴀʟʟ ᴘʀᴇᴍɪᴜᴍ ꜰᴇᴀᴛᴜʀᴇs!"
        )
    elif expiry_date_str == "Invalid Date Stored":
        status_msg = (
            f"⚠️ *{fancy('VIP Status Error')}*\n\n"
            f"ᴄᴏᴜʟᴅ ɴᴏᴛ ʀᴇᴀᴅ ʏᴏᴜʀ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ.\n"
            f"ᴘʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴛʜᴇ ᴀᴅᴍɪɴ ꜰᴏʀ ᴀssɪsᴛᴀɴᴄᴇ."
        )
    else:
        status_msg = (
            f"✅ *{fancy('Welcome Successfully')}!*\n\n"
            ...
            f"🏅 *sᴛᴀᴛᴜs:* 👤 ꜰʀᴇᴇ ᴜsᴇʀ\n"
            f"⏰ *ᴛɪᴍᴇ:* {time_str}\n\n"
            f"🚀 ᴜᴘɢʀᴀᴅᴇ ᴛᴏ *ᴠɪᴘ* ᴛᴏ ᴜɴʟᴏᴄᴋ:\n"
            f"  ⚡ ᴀᴜᴛᴏ ɢɪᴛʜᴜʙ ᴜᴘʟᴏᴀᴅs\n"
            f"  🔁 sᴄʜᴇᴅᴜʟᴇᴅ ꜰɪʟᴇ ᴘʀᴏᴄᴇssɪɴɢ\n\n"
            f"👉 ᴛᴀᴘ *🛒 ᴠɪᴘ sʜᴏᴘ* ᴛᴏ sᴇᴇ ᴘʟᴀɴs!"
        )
...
async def process_account(session: aiohttp.ClientSession, account: dict, semaphore: asyncio.Semaphore) -> tuple[str | None, str | None, dict | None, dict | None, str | None]:
    """
    Processes a single account using the embedded JWT generator.
    ...
    async with semaphore:
        try:
            ...
            token, region, error = await loop.run_in_executor(None, generate_jwt, str(uid), str(password))
            if token:
                logger.info(f"Success: Token received for UID: {uid} (Region: {region})")
                return token, region, original_account_info, None, None
            else:
                logger.warning(f"JWT generation failed for UID: {uid}. Error: {error}")
                lost_info = {**original_account_info, "error_reason": error or "Unknown error"}
                return None, None, None, lost_info, error
        except Exception as e:
            logger.error(f"Unexpected error processing UID {uid}: {e}", exc_info=True)
        ...
        try:
            ...
            if save_vip_data(vip_data):
                ...
                try:
                    ...
                    vip_dm_message = (
                        f"✅ *{fancy('Welcome Successfully')}!*\n\n"
                        f"👤 *ɴᴀᴍᴇ:* {fancy(str(target_user_id))}\n"
                        f"🎮 *ɪᴅ:* `{target_user_id}`\n"
                        f"🏅 *sᴛᴀᴛᴜs:* 👑 ᴠɪᴘ {'ᴜᴘᴅᴀᴛᴇᴅ' if is_extending else 'ᴀᴄᴛɪᴠᴀᴛᴇᴅ'}\n"
                        f"📅 *sᴜʙsᴄʀɪᴘᴛɪᴏɴ sᴛᴀʀᴛ:* `{added_on_str}`\n"
                        f"📅 *ᴇxᴘɪʀᴇs:* `{expiry_formatted_display}`\n"
                        f"👤 *ᴜᴘᴅᴀᴛᴇᴅ ʙʏ:* {admin_name}\n\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        f"⚡ ᴇɴᴊᴏʏ ᴀᴜᴛᴏ ɢɪᴛʜᴜʙ ᴜᴘʟᴏᴀᴅs & ꜰɪʟᴇ sᴄʜᴇᴅᴜʟɪɴɢ!"
                    )
...
async def main() -> None:
    ...
    logger.info("Using embedded JWT generator (OB53)")
    logger.info(f"OAuth URL: {OAUTH_URL}")
    logger.info(f"Major Login URL: {MAJOR_URL}")
    ...
    # Cancel Command & Button (works for both layouts)
    ...
    application.add_handler(MessageHandler(filters.Regex(f"^{re.escape(ADMIN_BUTTONS_LAYOUT[4][0])}$") & private_chat_filter, cancel))
    ...
    if ADMIN_ID and ADMIN_ID != 0:
        ...
        application.add_handler(MessageHandler(filters.Regex(f"^{re.escape(ADMIN_BUTTONS_LAYOUT[3][0])}$") & admin_filter, vip_management))
        application.add_handler(MessageHandler(filters.Regex(f"^{re.escape(ADMIN_BUTTONS_LAYOUT[3][1])}$") & admin_filter, broadcast))
        logger.info(f"Admin commands + buttons enabled for ADMIN_ID: {ADMIN_ID}.")
    else:
        logger.warning("Admin commands are disabled as ADMIN_ID is not set, 0, or invalid.")

    # Message Forwarding to Admin
    if ADMIN_ID and ADMIN_ID != 0:
        known_button_texts_set = (
            {btn for row in COMMAND_BUTTONS_LAYOUT for btn in row} |
            {btn for row in ADMIN_BUTTONS_LAYOUT for btn in row}
        )
        forwarding_filters = (
            private_chat_filter &
